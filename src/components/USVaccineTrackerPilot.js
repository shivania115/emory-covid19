  ...
  return (
    <div style={{ fontSize: "19px" }}>
      ...
      <center>
        <p>
          {" "}
          Percent vaccinated with at least 1 dose by Vulnerable Populations
        </p>
      </center>
      <LineChart
        width={720}
...
const CustomToolTipAgeGraph = ({ active, payload, label }) => {
  if (active && payload && payload.length) {
    return (
      <div className="custom-tooltip" style={{ lineHeight: "19px" }}>
...
const USVaccineTrackerPilot = (props) => {
  ...
  useEffect(() => {
    ...
    fetch("/data/vaccineSeriesAge.json")
      .then((res) => res.json())
      .then((x) => { setAgeSeries(x) });

    fetch('/data/vaccineAge.json')
      .then((res) => res.json())
      .then((x) => { setAgeVaccine(x) });
  ...
  const labelAge = (d) => {
    ...
    return (
      ...
      // </div>
    d.datum.childName=='Age 5-12'? (d.datum._x):("")+"\n"+
     d.datum.childName +" : " +(d.datum._y).toFixed(1) 
  ...
  if (
    ...
    console.log(ageSeries["_nation"].slice(1));
    const description = {
      aa:
              ...
              <Table celled inverted selectable>
                <Table.Body>
                  <Table.Row style={{ height: 25 }}>
                    ...
                    >
                      {numberWithCommas(
                        vaccineData[fips]["PercentAdministeredPartial"]
                      ) + "%"}
                    </Table.HeaderCell>
                  </Table.Row>
