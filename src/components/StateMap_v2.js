...
function SvgMap(props) {
    ...
    if (props.name === 'casescum') {
        return (
            <svg width="500" height="55">
                ...
                {_.map(colorPalette1, (color, i) => {
                    return <rect key={i} x={150 + 25 * i} y={20} width="25" height="20" style={{ fill: color, strokeWidth: 1, stroke: color }} />
                })}
                ...
                <text x={250} y={52} style={{ fontSize: '0.8em' }}>High</text>
                {_.map(props.legendSplit, (splitpoint, i) => {
                    if (props.legendSplit[i] < 1) {
                        return <text key={i} x={7 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {props.legendSplit[i].toFixed(1)}</text>
                    }
                    if (props.legendSplit[i] >= 1000) {
                        return <text key={i} x={20 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {(props.legendSplit[i] / 1000).toFixed(1) + "K"}</text>
                    }
                    return <text key={i} x={20 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {props.legendSplit[i].toFixed(0)}</text>
                })}

                {_.map(props.legendSplit1, (splitpoint, i) => {
                    if (props.legendSplit1[i] >= 1000) {
                        return <text key={i} x={170 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {(props.legendSplit1[i] / 1000).toFixed(1) + "K"}</text>
                    }
                    return <text key={i} x={170 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {props.legendSplit1[i].toFixed(0)}</text>
                })}

                <text x={0} y={15} style={{ fontSize: '0.7em' }}> {(props.legendMin / 100).toFixed(0)} </text>
                <rect x={290} y={20} width="25" height="20" style={{ fill: "#FFFFFF", strokeWidth: 0.5, stroke: "#000000" }} />
                <text x={320} y={27} style={{ fontSize: '0.7em' }}> None </text>
                <text x={320} y={40} style={{ fontSize: '0.7em' }}> Reported </text>
            </svg>
        ...
        return (
            <svg width="500" height="55">
                {_.map(colorPalette, (color, i) => {
                    return <rect key={i} x={5 + 25 * i} y={20} width="25" height="20" style={{ fill: color, strokeWidth: 1, stroke: color }} />
                })}

                <rect x={180} y={20} width="25" height="20" style={{ fill: colorOut, strokeWidth: 1, stroke: colorOut }} />
                <text x={5} y={52} style={{ fontSize: '0.8em' }}>Low</text>
                <text x={180} y={52} style={{ fontSize: '0.8em' }}>High</text>
                {_.map(props.legendSplit, (splitpoint, i) => {
                    if (props.legendSplit[i] < 1) {
                        return <text key={i} x={12 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {props.legendSplit[i].toFixed(1)}</text>
                    }
                    if (props.legendSplit[i] >= 1000) {
                        return <text key={i} x={22 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {(props.legendSplit[i] / 1000).toFixed(1) + "K"}</text>
                    }
                    return <text key={i} x={22 + 25 * (i)} y={15} style={{ fontSize: '0.7em' }}> {props.legendSplit[i].toFixed(0)}</text>
                })}
                {props.legendMin < 100 ? <text x={5} y={15} style={{ fontSize: '0.7em' }}> {(props.legendMin / 1).toFixed(0)} </text> :
                    <text x={-1} y={15} style={{ fontSize: '0.7em' }}> {(props.legendMin / 1).toFixed(0)} </text>
                }



                <text x={174} y={15} style={{ fontSize: '0.7em' }}>{props.legendSplit[colorPalette.length - 1] < 1 ? props.legendSplit[colorPalette.length - 1].toFixed(1)
                ...
                <text x={201} y={15} style={{ fontSize: '0.7em' }}>{props.legendMax}</text>
                <rect x={225} y={20} width="25" height="20" style={{ fill: "#FFFFFF", strokeWidth: 0.5, stroke: "#000000" }} />
                <text x={255} y={27} style={{ fontSize: '0.7em' }}> None </text>
                <text x={255} y={40} style={{ fontSize: '0.7em' }}> Reported </text>
        ...
        <VictoryChart theme={VictoryTheme.material}
            ...
            {varGraphPair[metric]['name'][1] === 'casesdailymean7' || varGraphPair[metric]['name'][1] === 'deathsdailymean7' ?
                <VictoryAxis dependentAxis tickCount={5}
                    ...
                    tickFormat={(y) => (y < 1000 ? y : (y / 1000 + 'k'))}
                /> :
                <VictoryLine name="Line11" style={{ data: { stroke: 'blue', strokeWidth: 3 } }} data={_.takeRight(props.data2[stateFips],14)  ? _.takeRight(props.data2[stateFips],14) : props.data2["99999"]}
                    x='t' y={varGraphPair[metric]['name'][1]} />}
        </VictoryChart>)
    }
    else {
        ...
        <VictoryChart theme={VictoryTheme.material}
            ...
            {varGraphPair[metric]['name'][1] === 'casesdailymean7' || varGraphPair[metric]['name'][1] === 'deathsdailymean7' ?
                <VictoryAxis dependentAxis tickCount={5}
                    ...
                    tickFormat={(y) => (y < 1000 ? y : (y / 1000 + 'k'))}
                /> :
                <VictoryLine name="Line11" style={{ data: { stroke: 'blue', strokeWidth: 3 } }} data={dataTS[stateFips] ? dataTS[stateFips] : dataTS["99999"]}
                    x='t' y={varGraphPair[metric]['name'][1]} />}
                                            ...
                                            <Grid.Row style={{ paddingLeft: "1.5", paddingTop: "1", paddingBottom: 0 }} centered>
                                                ...
                                                <svg width="630" height='80'>
                                                    ...
                                                    {varGraphPair[metric]['name'][1] === 'casesdailymean7' || varGraphPair[metric]['name'][1] === 'deathsdailymean7' ?
                                                        ...
                                                        <rect x={50} y={35} width="15" height="1" style={{ fill: 'blue', strokeWidth: 1, stroke: 'blue' }} />}
                                                    {varGraphPair[metric]['name'][1] === 'casesdailymean7' || varGraphPair[metric]['name'][1] === 'deathsdailymean7' ?
